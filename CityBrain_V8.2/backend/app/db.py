...
def init_db():
    ...
    if cur.fetchone()["c"] == 0:
        ...
        cur.execute("INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)", ("operator", hash_password("change_this_in_local_only"), "operator"))
        cur.execute("INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)", ("viewer", hash_password("change_this_in_local_only"), "viewer"))
...
def apply_sale(menu_name: str, sold_delta: int, congestion_level: Optional[str] = None, wait_minutes: Optional[int] = None, source: str = "manual"):
    state = get_state()
...
def _ensure_v4_tables():
    ...
    if cur.fetchone()["c"] == 0:
        cur.execute(
            ...
            ("20260001", "시연학생", "컴퓨터공학과", hash_password("change_this_in_local_only"), now_text())
        )
    conn.commit()
    conn.close()
